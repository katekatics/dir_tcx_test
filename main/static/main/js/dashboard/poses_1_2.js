function pos() {
    ...
    $.post("/store/" + full_sap + "/poses/", { csrfmiddlewaretoken: getCookie('tcx_token') },
        function (data) {
            if ("poses" in data) {
                var poses = '<div class="' + data.theme + '" id="poses_theme" tabindex="0" data-toggle="collapse" data-target="#poses" aria-expanded="true" aria-controls="poses">';
                ...
                $.each(data.poses, function (index, pos) {
                    poses += '<div class="col-xl-2 col-lg-3">';
                    if (pos.theme != "card bg-dark text-white my-1") {
                        poses += '<div class="' + pos.theme + '" tabindex="0" data-toggle="popover" data-placement="top" data-html="true" data-content="' + pos.popover + '">';
                    } else {
                        poses += '<div class="' + pos.theme + '">';
                    }
                    poses += '<div class="card-body-p0">';
                    poses += '<p class="text-center mb-0">' + pos.name + ' </p></div></div></div>';
                })
                poses += '</div></div></div>';
